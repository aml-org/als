...
import amf.plugins.domain.webapi.models.{EndPoint, Operation, Parameter, Request}
...
object ParameterStructure extends AMLCompletionPlugin {
  ...
  private def suggestions(request: AmlCompletionRequest, withName: Boolean, schema: Option[Shape]) = {
    val common = OasTypeFacetsCompletionPlugin.resolveShape(schema.getOrElse(ScalarShape()), Nil, request.indentation)

    val particular =
      if (withName) Seq(onlyBinding(request.indentation)) else paramsFields(request.indentation)
    particular ++ common
  }
  ...
  private def onlyBinding(indentation: String) =
    Oas20DialectWrapper.paramBiding.toRaw(indentation, CategoryRegistry(ParameterModel.`type`.head.iri(), "in"))

  private def paramsFields(indentation: String): Seq[RawSuggestion] =
    Oas20DialectWrapper.basicParamsProps
      .map(p => p.toRaw(indentation, CategoryRegistry(ParameterModel.`type`.head.iri(), p.name().value())))
}
