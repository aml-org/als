...
class MasterReferenceCompletionPlugin extends ICompletionPlugin {
  ...
  override def isApplicable(request: ICompletionRequest): Boolean = request.config.astProvider match {
    case Some(astProvider) =>
      languages.indexOf(astProvider.language) >= 0 && isExtendable(request) && isInExtendsProperty(request);
      ...
      PathCompletion
        ...
        .map(paths => {
          var suggestions = paths.map(path => {
            ...
            Suggestion(pathStartingWithPrefix, "master path", pathStartingWithPrefix, request.prefix)
          })
