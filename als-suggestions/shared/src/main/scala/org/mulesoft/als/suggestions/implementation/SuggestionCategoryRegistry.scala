...
private class SuggestionCategoryEntry(val text: String,
                                      val parentIs: Seq[String],
                                      val is: Seq[String],
...
object SuggestionCategoryRegistry {
  ...
  def getCategory(vendor: Vendor,
                  text: String,
                  owner: Option[ITypeDefinition],
                  range: Option[ITypeDefinition]): Option[String] =
    map.get(vendor.toString).flatMap(_.category(text, owner, range))
    ...
    buildYamlDocument(content).foreach(yDoc =>
      YJSONWrapper(yDoc.node).properties.foreach(cat => {
        ...
        cat.value.properties.foreach(prop => {
          var pName = prop.name
          var parentIs: Seq[String] = prop.value
            .propertyValue("parentIs", ARRAY)
            .map(_.map(_.value(STRING).orNull))
            .getOrElse(Seq())
            .filter(x => Option(x).isDefined)
          var is: Seq[String] = prop.value
            .propertyValue("is", ARRAY)
            .map(_.map(_.value(STRING).orNull))
            .getOrElse(Seq())
            .filter(x => Option(x).isDefined)
