...
class ExampleCompletionPlugin extends ICompletionPlugin {
	...
	def findProperties(path: Seq[String], localType: Option[ITypeDefinition], request:ICompletionRequest): Seq[ISuggestion] = if(localType.isEmpty) {
        ...
        localType.get.allProperties.filter(item => !siblingNames.contains(item.nameId.get)).map(prop => {
		...
		request.astNode match {
			case Some(requestNode) => {
				requestNode.parent match {
					case Some(parent) => if(parent.sourceInfo.yamlSources.length > 0) {
						parent.sourceInfo.yamlSources.head match {
							case rootNode: YNode => yNodeByPath(rootNode, path) match {
								case Some(ynodeValue: YNode) => ynodeValue.value match {
									case value: YMap => value.entries.map(entry => entry.key.value match {
	...
	def yNodeByPath(root: YNode, path: Seq[String]): Option[YNode] = {
		...
		root.value match {
			case value: YMap => value.entries.find(entry => entry.key.value match {
				case scalar: YScalar => current.equals(scalar.text);
				
				case _ => false;
			}) match {
				case Some(entry) => entry.value match {
	...
	def extractExampleNode(request: ICompletionRequest): Option[IHighLevelNode] = if(request.astNode.get.isElement) request.astNode.get.asElement else request.astNode.get.parent;
