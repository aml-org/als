...
import org.mulesoft.als.suggestions.{RawSuggestion, StringScalarRange, SuggestionOptions}
        ...
        valSuggestions(request.amfObject, request.fieldEntry, request.position.column).map(
          s =>
            RawSuggestion(
              ...
              s._2,
              Nil,
              options = SuggestionOptions(rangeKing = StringScalarRange, isKey = request.yPartBranch.isKey)))
      } else Nil
    }
  }
