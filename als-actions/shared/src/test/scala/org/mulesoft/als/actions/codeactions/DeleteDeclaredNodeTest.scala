...
import org.mulesoft.als.actions.codeactions.plugins.declarations.{DeleteDeclaredNodeCodeAction, ExtractElementCodeAction}
...
//            DeleteDeclaredNodeRequestCase("callback", ((54, 8), (54, 8)))
  ...
  it should s"delete node at aml document" in {
    val cases = Seq(DeleteDeclaredNodeRequestCase("dec1", ((2, 4), (2, 4))),
  ...
  case class DeleteResultCase(name:String, actions :Seq[CodeAction]) {
    ...
    private def buildAction(a:CodeAction,p:PartBuilder): Unit = {
      p.obj{eb =>
        a.edit.foreach { e => WorkspaceEditSerializer(e).entryChanges(eb) }
      }
    }
  }

  private def runCase(file:String, golden:String,cases:Seq[DeleteDeclaredNodeRequestCase], activeFile:Option[String]=None, dialectUri:Option[String] = None): Future[Assertion] = {
  ...
  private def getNodeDeletions(file:String, cases:Seq[DeleteDeclaredNodeRequestCase],activeFile:Option[String], defineBy:Option[String]= None) = {
    ...
    parseElement(file,defineBy).map(r => buildPreParam(file,r)).flatMap { pr =>
      val results: Seq[Future[DeleteResultCase]] = cases.map { deletecase =>
        ...
        DeleteDeclaredNodeCodeAction(params).run(params).map(ca => DeleteResultCase(deletecase.name, ca))
      }
