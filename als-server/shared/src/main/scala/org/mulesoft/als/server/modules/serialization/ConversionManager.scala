...
class ConversionManager(workspaceManager: WorkspaceManager, amfInstance: AmfInstance, logger: Logger)
  ...
  private def onSerializationRequest(uri: String, target: String, syntax: Option[String]): Future[SerializedDocument] = {
    workspaceManager.getUnit(uri, UUID.randomUUID().toString).flatMap(_.getLast) flatMap { cu =>
