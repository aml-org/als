...
trait AbstractServerConnection extends IServerConnection {
  ...
  protected var openDocumentListeners: Buffer[(IOpenedDocument) => Unit] = ArrayBuffer()

  protected var changeDocumentListeners: Buffer[(IChangedDocument) => Unit] = ArrayBuffer()

  protected var closeDocumentListeners: Buffer[(String) => Unit] = ArrayBuffer()

  protected var documentStructureListeners: Buffer[(String) => Future[Map[String, StructureNodeJSON]]] = ArrayBuffer()

  protected var documentCompletionListeners: Buffer[(String, Int) => Future[Seq[ISuggestion]]] = ArrayBuffer()

  protected var openDeclarationListeners: Buffer[(String, Int) => Future[Seq[ILocation]]] = ArrayBuffer()

  protected var findReferencesListeners: Buffer[(String, Int) => Future[Seq[ILocation]]] = ArrayBuffer()

  protected var markOccurrencesListeners: Buffer[(String, Int) => Future[Seq[IRange]]] = ArrayBuffer()

  protected var renameListeners: Buffer[(String, Int, String) => Future[Seq[IChangedDocument]]] = ArrayBuffer()

  protected var documentDetailsListeners: Buffer[(String, Int) => Future[Seq[ISuggestion]]] = ArrayBuffer()

  protected var changeDetailValueListeners: Buffer[(String, Int, String, AnyVal) => Future[Seq[IChangedDocument]]] =
    ArrayBuffer()

  protected var changePositionListeners: Buffer[(String, Int) => Unit] = ArrayBuffer()

  protected var serverConfigurationListeners: Buffer[(IServerConfiguration) => Unit] = ArrayBuffer()

  protected var calculateEditorContextActionsListeners: Buffer[(String, Int) => Future[Seq[IExecutableAction]]] =
    ArrayBuffer()

  protected var getAllEditorContextActionsListeners: Buffer[() => Future[Seq[IExecutableAction]]] = ArrayBuffer()

  protected var executeContextActionListeners: Buffer[(String, String, Int) => Future[Seq[IChangedDocument]]] =
    ArrayBuffer()

  protected var executeDetailsActionListeners: Buffer[(String, String, Int) => Future[Seq[IChangedDocument]]] =
    ArrayBuffer()
