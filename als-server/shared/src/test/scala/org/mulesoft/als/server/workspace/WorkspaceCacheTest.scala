...
class WorkspaceCacheTest extends AsyncFunSuite with Matchers with PlatformSecrets {
  ...
  test("test cache unit simple") {
    ...
    for {
      ws <- WorkspaceContentManager(folderUri, env, DummyTelemetryProvider, EmptyLogger, Nil)
      _ <- ws
  ...
  test("test cache unit sub ref") {
    ...
    for {
      ws <- WorkspaceContentManager("folder", env, DummyTelemetryProvider, EmptyLogger, Nil)
      _ <- ws
