...
class WEBLSPServerConnection() extends WEBLSPMessageDispatcher with AbstractServerConnection with IPrintlnLogger {
  ...
  protected def initialize(): Unit = {
    WorkerGlobals.addEventListener("message",
                                   (message: JSWorkerMessage) => this.handleJSONMessageRecieved(message.data));

    this.newVoidHandler("OPEN_DOCUMENT",
                        this.handleOpenDocument _,
                        Option(NodeMsgTypeMeta("org.mulesoft.language.server.js.OpenedDocument")));

    this.newVoidHandler("CHANGE_DOCUMENT",
                        this.handleChangedDocument _,
                        Option(NodeMsgTypeMeta("org.mulesoft.language.server.js.ChangedDocument")));

    this.newVoidHandler("CHANGE_POSITION",
                        this.handleChangedPosition _,
                        Option(NodeMsgTypeMeta("org.mulesoft.language.server.js.ChangedPosition")));

    this.newFutureHandler("GET_STRUCTURE",
                          this.handleGetStructure _,
                          Option(NodeMsgTypeMeta("org.mulesoft.language.server.js.GetStructure", true, true)));

    this.newVoidHandler("SET_LOGGER_CONFIGURATION",
                        this.handleSetLoggerConfiguration _,
                        Option(NodeMsgTypeMeta("org.mulesoft.language.server.js.LoggerSettings")));
  }
