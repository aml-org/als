...
class VSCConnection extends js.Any {
  ...
  def onDocumentSymbol(arg: js.Function1[JSDocumentSymbolParams, js.Promise[js.Array[JSSymbolInformation]]]): js.Any =
    js.native;
...
class MSLSPServerProcess extends js.Object {
  var vscConnection: VSCConnection =
    VSCodeServer.createConnection(new JSIPCMessageReader(Globals.process), new JSIPCMessageWriter(Globals.process));
  ...
  def start() {
    ...
    var server = new core.Server(this.scalaConnection, MSLSPHttpFetcher);
    ...
    this.documents.onDidChangeContent((event: JSDocumentChange) => {
      var changedDocument =
        new ChangedDocument(event.document._uri, event.document._version, Some(event.document._content), None);

      scalaConnection.handleChangedDocument(changedDocument);
    });

    this.scalaConnection.onValidationReport((report: ValidationReport) => {
      val diagnosticsByUri: scala.collection.mutable.Map[String, js.Array[Diagnostic]] =
        scala.collection.mutable.Map[String, js.Array[Diagnostic]]();
    ...
    this.vscConnection.onDocumentSymbol((symbolParams: JSDocumentSymbolParams) => {
      ...
      future
        .flatMap((structureResponse: GetStructureResponse) => {
          concurrent.Future {
