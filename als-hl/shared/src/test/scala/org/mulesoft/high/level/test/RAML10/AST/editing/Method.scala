...
class Method extends RAML10ASTEditingTest {
  ...
  test("Method. Editing used parametrized trait 1.") {
    runAttributeEditingTest(
      "Method/method_is_parametrized.raml",
      project => {
        project.rootASTUnit.rootNode
          .elements("resources")
          .head
    ...
    parse(filePath(fp))
      ...
      .flatMap(r => {
        if (r.result == succeed) {
          ...
          var traitNode =
            project.rootASTUnit.rootNode.elements("resources").head.elements("methods").head.elements("is")(1)
          var traitDef       = traitNode.definition
          ...
          runAttributeCreationTest1Internal(
            project,
            project => {
              Some(
                project.rootASTUnit.rootNode
      ...
      .map(r => {
        if (r.result != succeed) {
          ...
          var project = r.modifiedProject
          var traitParameters = project.rootASTUnit.rootNode
            .elements("resources")
            .head
            .elements("methods")
            .head
          ...
          var param1Value_ =
            traitParameters(0).attribute("value").get.value.get.asInstanceOf[JSONWrapper].value(STRING).get
          var param2Value_ =
            traitParameters(1).attribute("value").get.value.get.asInstanceOf[JSONWrapper].value(STRING).get
  ...
  ignore("Method. Creating new body and a property for it.") {
    parse(filePath("Method/method_method.raml"))
      .flatMap(project => {
        ...
        runAttributeCreationTest1Internal(
          ...
          project => {
            Some(
              project.rootASTUnit.rootNode.elements("resources").head.elements("methods").head.elements("body").head)
          },
          "name",
          "application/json"
        )
  ...
  ignore("Method queryString editing") {
    runAttributeEditingTest(
      "Method/query_string.raml",
      project => {
        project.rootASTUnit.rootNode
          .elements("resources")
          .head
...
//    }
  ...
  test("Method responses creation") {
    ...
    parse(filePath(fp)).flatMap(project => {
      ...
      runAttributeCreationTest1(project, project => {
        Some(
          project.rootASTUnit.rootNode.elements("resources").head.elements("methods").head.elements("responses").head)
      }, "code", "200")
    })
  }
...
//    }
  ...
  test("Method securedBy creation") {
    ...
    parse(filePath(fp)).flatMap(project => {
      ...
      runAttributeCreationTest1(project, project => {
        Some(
          project.rootASTUnit.rootNode.elements("resources").head.elements("methods").head.elements("securedBy").head)
      }, "name", "oauth")
    })
  }
