...
import org.mulesoft.high.level.interfaces.{IASTUnit, IFSProvider, IProject}
import org.mulesoft.typesystem.project.ITypeCollectionBundle

import scala.collection.mutable

class Project(bundle:ITypeCollectionBundle, _lang:Vendor, _fsProvider:IFSProvider) extends IProject {
    ...
    override def fsProvider: IFSProvider = _fsProvider
    ...
    override def resolvePath(absBasePath: String, path: String): Option[String] = _fsProvider.resolve(absBasePath, path)
}

object Project {

    def apply(bundle:ITypeCollectionBundle, lang:Vendor, fsResolver:IFSProvider): Project = new Project(bundle,lang,fsResolver)
}
