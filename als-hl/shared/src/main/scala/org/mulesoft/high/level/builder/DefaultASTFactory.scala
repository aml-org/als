...
abstract class DefaultASTFactory extends IASTFactory {
  ...
  private def registerObject[T](typeName: String,
                                propertyName: String,
                                obj: T,
  ...
  private def getObjectByTypeName[T](typeName: String,
                                     propertyName: String,
  ...
  private def getObject[T](t: ITypeDefinition,
                           propertyName: String,
                           map: mutable.Map[String, mutable.Map[String, T]]): Option[T] = {
  ...
  override def getPropertyValues(node: IHighLevelNode,
                                 prop: IProperty,
                                 bundle: ITypeCollectionBundle): Seq[BasicASTNode] =
    ...
    var resultOpt = for {
      ...
      propertyMatches.flatMap({
        case am: AttributeMatchResult =>
          val attr =
            ASTPropImpl(am.node, baseUnit, Option(node), discriminate(range, am.node, None), Option(prop), am.buffer)
          Some(attr)
        case em: ElementMatchResult =>
          val nominalType = determineUserType(em.node, Option(prop), Option(node), bundle)
          var hlNode =
            ASTNodeImpl(em.node, baseUnit, Option(node), discriminate(range, em.node, nominalType), Option(prop))
    ...
    var resultOpt = for {
      ...
      propertyMatches.flatMap({
        ...
        case fmr: FieldOwnerMatchResult =>
          ...
          var hlNode =
            ASTNodeImpl(newNode, baseUnit, Option(node), discriminate(range, newNode, nominalType), Option(prop))
          //em.yamlNode.foreach(x => hlNode.sourceInfo.withSources(List(x)))
          nominalType.foreach(hlNode.setLocalType)
          initNode(hlNode)
          Some(hlNode)
...
object DefaultASTFactory {
  ...
  def extractShape(obj: AmfObject): Option[Shape] = {
    ...
    var dElementOpt: Option[DomainElement] = obj match {
      case bu: BaseUnit =>
        Option(bu.fields.getValue(DocumentModel.Encodes))
          .map(_.value)
          .map({
    ...
    if (dElementOpt.isEmpty) {
      ...
      var shapeOpt: Option[Shape] = dElement.meta match {
        case PropertyShapeModel =>
          Option(
            dElement.fields
              .get(PropertyShapeModel.Range)
              .asInstanceOf[Shape])
        case CustomDomainPropertyModel =>
          Option(dElement.fields.get(CustomDomainPropertyModel.Schema).asInstanceOf[Shape])
...
object FieldMatchResult {
  def apply(node: AmfObject, value: AmfElement, fMatcher: FieldMatcher): FieldMatchResult =
    new FieldMatchResult(node, value, fMatcher)
}
...
object FieldOwnerMatchResult {
  def apply(fieldOwner: AmfObject, fMatcher: FieldMatcher): FieldOwnerMatchResult =
    new FieldOwnerMatchResult(fieldOwner, fMatcher)
}
        ...
        if (yp.lengthCompare(1) == 0) {
          ...
          result = result.flatMap({
            case er: ElementMatchResult =>
              var srcOpt = er.yamlNode
                .flatMap(YJSONWrapper(_))
                .flatMap(_.propertyValue(key))
                .map(_.source)
              srcOpt match {
      ...
      case Some(c) =>
        result = result.map({
          case fmr: FieldMatchResult =>
            AttributeMatchResult(fmr.node, c(fmr.value.asInstanceOf[AmfObject], cfg.hlNode))
...
class BaseUnitMatcher extends IPropertyMatcher {
  ...
  override def doOperate(obj: AmfObject, hlNode: IHighLevelNode): Seq[MatchResult] =
    List(ElementMatchResult(hlNode.amfBaseUnit))

  override def doAppendNewValue(cfg: NodeCreationConfig): Option[MatchResult] =
    Some(ElementMatchResult(cfg.hlNode.amfBaseUnit))
}
...
class AndMatcher(matchers: Seq[IPropertyMatcher]) extends IPropertyMatcher {
  ...
  override def doOperate(obj: AmfObject, hlNode: IHighLevelNode): Seq[MatchResult] = {
    ...
    if (matchers.isEmpty) {
      ...
      var table: Seq[ListBuffer[MatchResult]] = matchers.map(ListBuffer[MatchResult]() ++= _.operate(obj, hlNode))
      var size = table.foldLeft(0) { (x, i) =>
        math.max(x, i.length)
      }
  ...
  override def doAppendNewValue(cfg: NodeCreationConfig): Option[MatchResult] = {
    if (matchers.isEmpty) {
      ...
      var results: Seq[AttributeMatchResult] = matchers
        .flatMap(_.appendNewValue(cfg))
        .filter(_.isInstanceOf[AttributeMatchResult])
        .map(_.asInstanceOf[AttributeMatchResult])
    ...
    obj.fields.get(field) match {
      case dataNode: DataNode =>
        dataNode.annotations.find(classOf[SourceAST]) match {
          case Some(yn) => result += AttributeMatchResult(obj, JSONValueBuffer(obj, hlNode, YJSONWrapper(yn.ast)))
          case _        =>
        }

      case ex: Examples =>
        ex.examples.foreach(x => {
          result += ElementMatchResult(x)
        })
      case amfObj: AmfObject => result += ElementMatchResult(amfObj)
      case scalar: AmfScalar =>
        result += AttributeMatchResult(obj, bufferConstructor match {
...
class NodeCreationConfig(val obj: AmfObject,
  ...
  def newConfigWithObject(o: AmfObject): NodeCreationConfig =
    new NodeCreationConfig(o, hlNode, prop, factory, typeHint)
...
object NodeCreationConfig {
  def apply(obj: AmfObject,
            hlNode: IHighLevelNode,
            prop: IProperty,
            factory: IASTFactory,
            typeHint: Option[ITypeDefinition] = None): NodeCreationConfig =
    new NodeCreationConfig(obj, hlNode, prop, factory, typeHint)
}
