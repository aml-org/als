...
class EmtyFileTest extends RAML10Test {
    ...
    test("Empty YAML file completion"){
        this.runTest("empty/test001.raml", Set("RAML 1.0", "RAML 0.8"))
    }
    ...
    test("'#%RAML 1.0' completion"){
        this.runTest("empty/test009.raml", Set(" ResourceType", " Trait", " AnnotationTypeDeclaration", " DataType", " DocumentationItem", " NamedExample", " Extension", " Extension", " Overlay", " Library"))
    }

    test("'#%RAML 1.0\\SPACE' completion"){
        this.runTest("empty/test010.raml", Set("ResourceType", "Trait", "AnnotationTypeDeclaration", "DataType", "DocumentationItem", "NamedExample", "Extension", "Extension", "Overlay", "Library"))
    }
