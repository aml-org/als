...
import org.mulesoft.language.server.server.modules.commonInterfaces.{IPoint, IRange}
...
class ChangeDocumentTest extends LanguageServerTest{
    ...
    test("chande document test 001"){
        ...
        getClient.flatMap(client=>{
            ...
            client.documentChanged(IChangedDocument(url,1,Some(content2),None))

            org.mulesoft.high.level.Core.init()

            getClient.flatMap(client=>{
                client.getStructure(url).map(actualOutline=>{
                    client.documentClosed(url)
                    actualOutline.get("SchemasAndTypesCategory").flatMap(_.children.headOption).map(_.text).map({
                        case "MyType" => succeed
