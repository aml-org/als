...
override def stop(): Unit = {}
    ...
    /**
      ...
      * @param uri
      */
    override def getStructure(uri: String): Future[Map[String, SharedStructureNode]] = this.sendWithResponse[GetStructureResponse]("GET_STRUCTURE", GetStructureRequest(uri)).map(x=>x.wrapped.map(e=>(e._1,StructureNode.transportToShared(e._2))))
    ...
    /**
      ...
      */
    override def getSuggestions(uri: String, position: Int): Future[Seq[ISuggestion]] = this.sendWithResponse[GetCompletionResponse]("GET_SUGGESTIONS", GetCompletionRequest(uri,position)).map(_.suggestions.map(Suggestion.transportToShared))
    ...
    /**
      ...
      * @param position - position in the document
      */
    override def findReferences(uri: String, position: Int): Future[Seq[ILocation]] = this.sendWithResponse[LocationsResponse]("FIND_REFERENCES", FindReferencesRequest(uri,position)).map(r=>r.wrapped.map(Location.transportToShared))
    ...
    def EXISTS(path:String): Future[Boolean] = Future.sequence(onExistsListeners.map(x=>x(path))).map(_.exists(x=>x))
    ...
    def IS_DIRECTORY(path: String): Future[Boolean] = Future.sequence(onIsDirectoryListeners.map(x=>x(path))).map(_.exists(x=>x))
    ...
    /**
      ...
      *                 If not provided, the last reported by positionChanged method will be used.
      */
    override def executeContextAction(uri: String, action: IExecutableAction, position: Int): Future[Seq[IChangedDocument]] = {
        Future.successful(Seq())
    }
    ...
    /**
      ...
      */
    override def changeDetailValue(uri: String, position: Int, itemID: String, value: AnyVal): Future[Seq[IChangedDocument]] = {
