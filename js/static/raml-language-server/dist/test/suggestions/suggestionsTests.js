...
function completionByUniqueEntryAsync(filePath, entry, begin, callback) {
    ...
    var position = begin ? (content.indexOf(entry)) : offsetForEntry(entry, content);
    ...
    connection.getSuggestions(filePath, position).then(function (result) {
        connection.documentClosed(filePath);
        callback(result.map(function (suggestion) { return suggestion.displayText || suggestion.text; }));
    });
}
exports.completionByUniqueEntryAsync = completionByUniqueEntryAsync;
function testCompletionByEntryStart(testPath, done, entry, expected) {
    completionByUniqueEntryAsync(testPath, entry, true, function (result) {
        try {
...
function testCompletionByEntryEnd(testPath, done, entry, expected) {
    completionByUniqueEntryAsync(testPath, entry, false, function (result) {
        try {
            assert(compareProposals(result, expected));
            done();
        }
...
function compareProposals(actualProposals, expectedStr) {
    var expectedProposals = expectedStr.length === 0 ? [] : expectedStr.split(", ");
    ...
    for (var _i = 0, expectedProposals_1 = expectedProposals; _i < expectedProposals_1.length; _i++) {
        var expectedProposal = expectedProposals_1[_i];
...
//# sourceMappingURL=suggestionsTests.js.map