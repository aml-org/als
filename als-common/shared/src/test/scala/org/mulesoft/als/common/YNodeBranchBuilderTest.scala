...
class YNodeBranchBuilderTest extends FunSuite with Matchers {
  ...
  test("Detect if we are in a key or not YAML") {
    ...
    val aBranch = NodeBranchBuilder.build(root, inKey1, isJson = false)
    aBranch.isKey should be(true)

    val subKey1 = AmfPosition(2, 3)
    val bBranch = NodeBranchBuilder.build(root, subKey1, isJson = false)
    bBranch.isKey should be(true)

    val subKey2 = AmfPosition(3, 4)
    val cBranch = NodeBranchBuilder.build(root, subKey2, isJson = false)
    cBranch.isKey should be(true)

    val inSubKey3 = AmfPosition(8, 7)
    val dBranch   = NodeBranchBuilder.build(root, inSubKey3, isJson = false)
  ...
  test("Detect if we are in an array") {
    ...
    val aBranch = NodeBranchBuilder.build(root, inArray, isJson = false)
    aBranch.isInArray should be(true)

    val inArray2 = AmfPosition(6, 8)
    val bBranch  = NodeBranchBuilder.build(root, inArray2, isJson = false)
    bBranch.isInArray should be(false)

    val inArray3 = AmfPosition(5, 8)
    val cBranch  = NodeBranchBuilder.build(root, inArray3, isJson = false)
    cBranch.isInArray should be(true)

    val inArray4 = AmfPosition(7, 13)
    val dBranch  = NodeBranchBuilder.build(root, inArray4, isJson = false)
    dBranch.isInFlow should be(true)
  }
  ...
  test("Detect if we are in root") {
    ...
    val aBranch = NodeBranchBuilder.build(root, key, isJson = false)
    assert(aBranch.isAtRoot)

    val key2    = AmfPosition(11, 1)
    val bBranch = NodeBranchBuilder.build(root, key2, isJson = false)
    assert(bBranch.isAtRoot)

    val subkey8 = AmfPosition(10, 5)
    val cBranch = NodeBranchBuilder.build(root, subkey8, isJson = false)
  ...
  test("Detect brothers in different levels") {
    ...
    val sk6Branch = NodeBranchBuilder.build(root, subKey6, isJson = false)
    assert(sk6Branch.brothersKeys.head == "subKey7")

    val scalar2 = AmfPosition(10, 9)
    val bBranch = NodeBranchBuilder.build(root, scalar2, isJson = false)
  ...
  test("Detect indentation in different levels") {
    ...
    val param1Brother = NodeBranchBuilder.build(root, AmfPosition(6, 10), isJson = false)
    assert(param1Brother.node.isInstanceOf[YMap])
    assert(param1Brother.brothersKeys.contains("param1"))

    val subKey3BrotherBranch = NodeBranchBuilder.build(root, AmfPosition(10, 6), isJson = false)
    assert(subKey3BrotherBranch.node.isInstanceOf[YMap])
    assert(subKey3BrotherBranch.brothersKeys.contains("subKey3"))

    val param5BrotherBranch = NodeBranchBuilder.build(root, AmfPosition(10, 8), isJson = false)
    assert(param5BrotherBranch.node.isInstanceOf[YMap])
    assert(param5BrotherBranch.brothersKeys.contains("param4"))

    val rootBranch = NodeBranchBuilder.build(root, AmfPosition(10, 0), isJson = false)
    assert(rootBranch.node.isInstanceOf[YMap])
    assert(rootBranch.isAtRoot)

    val anotherRootBranch = NodeBranchBuilder.build(root, AmfPosition(6, 0), isJson = false)
  ...
  test("Json Range for empty key with brothers") {
    ...
    val branch   = NodeBranchBuilder.build(root, position, isJson = true)
    assert(branch.node.isInstanceOf[YMap])
  }
