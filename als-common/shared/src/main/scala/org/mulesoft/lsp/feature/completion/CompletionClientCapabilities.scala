...
/**
  ...
  *                            capabilities.
  * @param completionItemKind  The client supports the following `CompletionItemKind` specific
  *                            capabilities
  * @param contextSupport      The client supports to send additional context information for a
  *                            `textDocument/completion` request.
  */
case class CompletionClientCapabilities(dynamicRegistration: Option[Boolean] = None,
...
/**
  * @param snippetSupport          The client supports snippets as insert text.
  *
  *                                A snippet can define tab stops and placeholders with `$1`, `$2`
  *                                and `${3:foo}`. `$0` defines the final tab stop, it defaults to
  *                                the end of the snippet. Placeholders with equal identifiers are linked,
  *                                that is typing in one will update others too.
  * @param commitCharactersSupport The client supports commit characters on a completion item.
  * @param deprecatedSupport       The client supports the deprecated property on a completion item.
  * @param preselectSupport        The client supports the preselect property on a completion item.
  */

case class CompletionItemClientCapabilities(snippetSupport: Option[Boolean],
                                            commitCharactersSupport: Option[Boolean],
                                            deprecatedSupport: Option[Boolean],
                                            preselectSupport: Option[Boolean])

/**
  * @param valueSet The completion item kind values the client supports. When this
